"""
   Ultra Low VRAM optimized version for RTX 4060 Ti 16GB
   Processes reference images in chunks using CPU RAM
   This allows using ALL 73 reference images without OOM errors
"""
...
parser = argparse.ArgumentParser(description='Remastering - Chunked References')
...
parser.add_argument('--ref_chunk_size', type=int, default='15', help='Number of reference images to process at once (lower = less VRAM)')
...
# Prepare reference images - KEEP IN CPU RAM
refimgs_chunks = []
if not opt.disable_colorization:
   if opt.reference_dir!='none':
      ...
      # Split references into chunks to fit in VRAM
      ...
      print(f'Reference chunks prepared in CPU RAM')
      ...
      # Perform colorization with chunked references
         ...
         if opt.reference_dir=='none':
            ...
            for chunk_idx, refimgs_chunk in enumerate(refimgs_chunks):
               # Move this chunk to GPU
               refimgs_gpu = refimgs_chunk.to(device)
